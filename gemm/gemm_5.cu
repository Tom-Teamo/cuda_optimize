...
/*
Note:
    __align__(16 * 1024)：将变量对齐到内存位置为16 * 1024的整数倍上
                          https://lecoding.com/post/2016/about-cpp-alignment/
...
__global__ void sgemm(int m, int n, int k, float *a, int lda, float *b, int ldb,
  ...
  for (float *a_ptr = begin_a, *b_ptr = begin_b; a_ptr < end_a;
    ...
    __shared__ __align__(4 * 4 * 1024) float ashare[STEP][STEP];
    __shared__ __align__(4 * 4 * 1024) float bshare[STEP][STEP];
